...
const getArc = (arcNumber, totalArc) => {
  ...
  const arc = d3.shape.arc().outerRadius(oRadius).innerRadius(iRadius).startAngle(radFrom).endAngle(radTo);
  return arc();
};
...
export default (props) => {
  ...
  const getPaths = (total, totalFrom) => {
    ...
    const to = isArray ? total.length : total;

    for (let i = 1; i <= to; i++) {
      paths.push(<path key={i} d={getArc(isArray ? total[i - 1] : i, totalFrom)} />);
    }
  ...
  return (
    <>
      ...
      <svg viewBox="0 0 100 100" className={styles.svg} fill="rgba(106, 164, 224, 0.12)">
